package io.github.luxmixus.autoconfigure.servlet;

import io.github.luxmixus.autoconfigure.ConditionalOnListProperty;
import io.github.luxmixus.autoconfigure.servlet.filter.RefererFilter;
import io.github.luxmixus.autoconfigure.servlet.filter.RepeatableFilter;
import io.github.luxmixus.autoconfigure.servlet.filter.XssFilter;
...
@ConditionalOnProperty(value = "luxmixus.servlet.auto", havingValue = "true", matchIfMissing = true)
public class ServletAutoConfiguration {
    ...
    static class XssFilterRegistrationConfiguration {
        @Bean
        @ConditionalOnListProperty(value = "luxmixus.servlet.filter.xss-includes")
        ...
        public FilterRegistrationBean<XssFilter> xssFilterRegistration(ServletFilterProperties filterProperties) {
            ...
            switch (filterProperties.getXssSanitizer()) {
                ...
                default:
                    sanitizer = s -> Jsoup.clean(s, Safelist.relaxed());
                    log.warn("Unsupported value '{}' for property 'luxmixus.servlet.filter.xss-sanitizer', " +
    ...
    @Bean
    @ConditionalOnListProperty(value = "luxmixus.servlet.filter.referer-allow-domains")
    ...
    @Bean
    @ConditionalOnProperty(value = "luxmixus.servlet.filter.repeatable", havingValue = "true")
