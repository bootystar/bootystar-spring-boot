package io.github.luminsyn.autoconfigure.servlet;

import io.github.luminsyn.autoconfigure.ConditionalOnListProperty;
import io.github.luminsyn.autoconfigure.servlet.filter.RefererFilter;
import io.github.luminsyn.autoconfigure.servlet.filter.RepeatableFilter;
import io.github.luminsyn.autoconfigure.servlet.filter.XssFilter;
...
@ConditionalOnProperty(value = "luminsyn.servlet.auto", havingValue = "true", matchIfMissing = true)
public class ServletAutoConfiguration {
    ...
    static class XssFilterRegistrationConfiguration {
        @Bean
        @ConditionalOnListProperty(value = "luminsyn.servlet.filter.xss-includes")
        ...
        public FilterRegistrationBean<XssFilter> xssFilterRegistration(ServletFilterProperties filterProperties) {
            ...
            switch (filterProperties.getXssSanitizer()) {
                ...
                default:
                    sanitizer = s -> Jsoup.clean(s, Safelist.relaxed());
                    log.warn("Unsupported value '{}' for property 'luminsyn.servlet.filter.xss-sanitizer', " +
    ...
    @Bean
    @ConditionalOnListProperty(value = "luminsyn.servlet.filter.referer-allow-domains")
    ...
    @Bean
    @ConditionalOnProperty(value = "luminsyn.servlet.filter.repeatable", havingValue = "true")
